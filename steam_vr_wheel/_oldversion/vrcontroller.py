...
class Controller:
    ...
    def update(self, pose):
        ...
        try:
            self.yaw = 180 / math.pi * math.atan2(pose_mat[1][0], pose_mat[0][0])
        except ZeroDivisionError:
            self.yaw = 0
        try:
            self.pitch = 180 / math.pi * math.atan2(
                -1 * pose_mat[2][0], math.sqrt(pow(pose_mat[2][1], 2) + math.pow(pose_mat[2][2], 2)))
        except ZeroDivisionError:
            self.pitch = 0

        try:
            self.roll = 180 / math.pi * math.atan2(pose_mat[2][1], pose_mat[2][2])
        except ZeroDivisionError:
            self.roll = 0
        self.triggerPressure = pControllerState.rAxis[1].x
        self.trackpadX = pControllerState.rAxis[0].x
        self.trackpadY = pControllerState.rAxis[0].y
        self.thumbstickX = pControllerState.rAxis[2].x
        self.thumbstickY = pControllerState.rAxis[2].y
