...
class WheelMapping(ControllerMapping):
    ...
    def generate_axis_mapping(self, root_node: VrSystemState) -> dict[int, Axis]:
        ...
        return {
            ...
            # note that we subtract the HMD roll, so you can tilt your entire body left/right without it causing your vehicle to steer
            HID_USAGE_RZ: GatedAxis(0.5)(
                ...
                ScaleAxis(HALF_CIRCLE_ROTATION_SCALAR, 0.0)(DifferenceAxis(Wheel(left_controller_state, right_controller_state), RollAxis(hmd_state)))
            ),
    ...
    def trackpad_edge_buttons(self, controller_state: ControllerStateGenerator, require_press = False) -> Iterable[Button]:
        base_buttons = [
            ...
            AxisThresholdButton(-TOUCHPAD_EDGE_BUTTON_THRESHOLD, '<')(ControllerAxis(0, 'y')(controller_state)), # touchpad bottom
            AxisThresholdButton(-TOUCHPAD_EDGE_BUTTON_THRESHOLD, '<')(ControllerAxis(0, 'x')(controller_state)), # touchpad left
        ]

        if require_press:
            return [AndButton(button, DirectButton(openvr.k_EButton_Axis0, 'press')(controller_state)) for button in base_buttons]
        
        return base_buttons
    
    def thumbstick_edge_buttons(self, controller_state: ControllerStateGenerator) -> Iterable[Button]:
        return [
            AxisThresholdButton(TOUCHPAD_EDGE_BUTTON_THRESHOLD, '>')(ControllerAxis(2, 'y')(controller_state)), # thumbstick top
            AxisThresholdButton(TOUCHPAD_EDGE_BUTTON_THRESHOLD, '>')(ControllerAxis(2, 'x')(controller_state)), # thumbstick right
            AxisThresholdButton(-TOUCHPAD_EDGE_BUTTON_THRESHOLD, '<')(ControllerAxis(2, 'y')(controller_state)), # thumbstick bottom
            AxisThresholdButton(-TOUCHPAD_EDGE_BUTTON_THRESHOLD, '<')(ControllerAxis(2, 'x')(controller_state)), # thumbstick left
        ]
    ...
    def generate_button_mapping(self, root_node: VrSystemState) -> dict[int, Button]:
        ...
        buttons = [
            ...
            self.sticky_forward_tilt(left_controller_grip, left_controller_pitch, hmd_pitch),    # primary/secondary fire by pressing left/right grip and tilting controller forward
            self.sticky_forward_tilt(right_controller_grip, right_controller_pitch, hmd_pitch),
            CircleGesture(True, TRANSLATIONAL_GESTURE_THRESHOLD, left_controller_x, left_controller_y, left_controller_grip),
            *self.trackpad_edge_buttons(left_controller_state, True),  # buttons triggered by touching the edges of the left trackpad
            *self.trackpad_edge_buttons(right_controller_state, True),  # buttons triggered by touching the edges of the right trackpad
            *self.thumbstick_edge_buttons(right_controller_state),  # buttons triggered by the cardinal directions of the right thumbstick
